...
class WrongFormulaUsageDetector : Detector(), Detector.UastScanner {
    ...
    override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
        ...
        if (evaluator.isMemberInClass(method, "com.instacart.formula.FormulaContext")) {
            checkInEvaluate(context, node)
            return
        }
        ...
        if (evaluator.isMemberInClass(method, "android.util.Log")) {
            context.report(
                Incident(
                    issue = ISSUE_FORMULA_CONTEXT_USAGE,
                    scope = node,
    ...
    private fun checkInEvaluate(context: JavaContext, call: UCallExpression) {
...
//            name = current?.is
            if (current == null || current is UMethod) {
                ...
                context.report(
                    Incident(
                        ...
                        message = "Cannot use FormulaContext once Formula.evaluate returns"
                    )
                )
            ...
            if (current.isMethodCall()) {
                ...
                if (psiMethod != null && isFormulaMember(psiMethod, context.evaluator)) {
                    return
                }
            }
        }
    ...
    private fun isFormulaMember(method: PsiMethod, evaluator: JavaEvaluator): Boolean {
        return evaluator.isMemberInClass(method, "com.instacart.formula.Formula")
            || evaluator.isMemberInClass(method, "com.instacart.formula.StatelessFormula")
    }
    ...
    companion object {
        ...
        val ISSUE_FORMULA_CONTEXT_USAGE = Issue.create(
            id = "InvalidFormulaContextUsage",
            briefDescription = "Using FormulaContext once Formula.evaluate() returns",
            explanation = "It is an error to use FormulaContext once Formula.evaluate() returns an Evaluation.",
            category = MESSAGES,
            priority = 5,
            severity = ERROR,
        ...
        val ISSUE_FORMAT = Issue.create(
            ...
            priority = 5,
            severity = WARNING,
            implementation = Implementation(WrongFormulaUsageDetector::class.java, JAVA_FILE_SCOPE)
        )
